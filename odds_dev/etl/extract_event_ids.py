...
that was previously generated by historical_events.py and extracts all event IDs
and their commence times, then saves them to storage for use by other processes.

Data Flow:
1. historical_events.py → Creates files: odds/historical_events/season_2024/raw_odds_historical_events_YYYY-MM-DD.json
2. extract_event_ids.py → Reads those files and creates: odds/historical_event_id/season_2024/raw_odds_historical_event_id_YYYY-MM-DD.json
3. Other processes can use these event IDs for historical odds extraction

Path Structure (aligned with historical_events.py):
- Bucket: SMARTBETTING_STORAGE
- Season: SEASON_2024
- Input Path: odds/historical_events/season_2024/
- Output Path: odds/historical_event_id/season_2024/
"""
...
class EventDataExtractor:
    ...
    def __init__(self):
        ...
        self.season = Season.SEASON_2024  # Changed from schema to season
        self.table = Table.HISTORICAL_EVENTS
        # Output path for saving event data
        self.output_catalog = Catalog.ODDS
        self.output_season = Season.SEASON_2024  # Changed from schema to season
    ...
    def list_historical_events_files(self) -> List[str]:
        ...
        try:
            ...
            prefix = f"{self.catalog}/{self.table}/{self.season}/"
    ...
    def read_historical_events_file(self, file_name: str) -> List[Dict[str, Any]]:
        ...
        as created by historical_events.py using smartbetting.convert_to_ndjson()
    ...
    def save_event_data_to_storage(
        ...
        filename = f"raw_{self.output_catalog}_{self.output_table}_{target_date.strftime('%Y-%m-%d')}.json"

        # Create the full GCS path
        gcs_path = (
            f"{self.output_catalog}/{self.output_table}/{self.output_season}/{filename}"
        )
